...
### download schedule
url <- read_html("https://lnb.com.br/nbb/tabela-de-jogos/?season%5B%5D=63")
...
schedule2122 <- tibble(id, home_away = rep(c("home","away"), length(date)/2), date, time, match, team, opponent, season_phase, season, venue)
...
links_2122 <- url %>% html_nodes(".match_score_relatorio") %>% html_attr("href")
links_2122 <- links_2122[c(T,F)]
links_2122 <- tibble(link = links_2122, id = c(1:length(links_2122)))
links_2122 <- links_2122[-c(115, 117, 246),] #delete WO
...
for (i in 1:nrow(links_2122)) {
  link <- links_2122$link[i]
  if (grepl("partida", link) == F) {
    ...
    idh <- paste0(links_2122$id[i], "H")
    ida <- paste0(links_2122$id[i], "A")
...
period2122 <- rbind(period_static, period_dynamic) %>% 
  arrange(id)
period2122 <- period2122 %>% 
  ...
  fill(game_result)
coach2122 <- rbind(coach_static, coach_dynamic) %>% 
  arrange(id)
...
nbb2122 <- rbind(dynamic_final, static_final)
...
home2122$id <- home2122$id %>%  str_replace_all("H","A")

away2122 <- team2122 %>% 
  subset(grepl("A", team2122$id)) %>% 
  rename(o_points_made = t_points_made,
         o_points_attempted = t_points_attempted,
         o_points_percentage = t_points_percentage,
         o_assists = t_assists,
...
away2122$id <- away2122$id %>%  str_replace_all("A","H")

opponent2122 <- rbind(home2122, away2122) %>% 
  arrange(id)

team2122 <- left_join(team2122, opponent2122) %>% 
  select(-name) %>% 
  arrange(id)

team2122 <- team2122 %>% ungroup() %>% 
  mutate(t_total_possessions = (((team2122$t_three_points_attempted + team2122$t_two_points_attempted +
                                    team2122$t_turnovers + (0.4*team2122$t_free_throws_attempted) -
                                    (team2122$t_offensive_rebounds/(team2122$t_offensive_rebounds + team2122$o_defensive_rebounds))*
                                    ((team2122$t_three_points_attempted + team2122$t_two_points_attempted) -
                                       (team2122$t_three_points_made + team2122$t_two_points_made))*1.07) + (
                                         team2122$o_three_points_attempted + team2122$o_two_points_attempted +
                                           team2122$o_turnovers + (0.4*team2122$o_free_throws_attempted) -
                                           (team2122$o_offensive_rebounds/(team2122$o_offensive_rebounds + team2122$t_defensive_rebounds))*
                                           ((team2122$o_three_points_attempted + team2122$o_two_points_attempted) -
                                              (team2122$o_three_points_made + team2122$o_two_points_made))*1.07 ))/2),
         
         t_scoring_possessions = (team2122$t_three_points_made + team2122$t_two_points_made +
                                    (1 - (1-team2122$t_free_throws_percentage)^2)*team2122$t_free_throws_attempted*0.4),
         
         t_floor_percentage = t_scoring_possessions/t_total_possessions,
         
         t_plays = (team2122$t_three_points_attempted + team2122$t_two_points_attempted +
                      team2122$t_free_throws_attempted*0.4 + team2122$t_turnovers),
         
         t_plays_percentage = t_scoring_possessions/t_plays,
         
         t_field_percentage = (team2122$t_three_points_made + team2122$t_two_points_made)/
           ((team2122$t_three_points_attempted + team2122$t_two_points_attempted - team2122$t_offensive_rebounds)/
              (team2122$t_offensive_rebounds + team2122$o_defensive_rebounds)*
              (team2122$t_three_points_attempted + team2122$t_two_points_attempted -
                 team2122$t_three_points_made - team2122$t_two_points_made)*1.07 +
              team2122$t_turnovers),
         
         t_offensive_rating = team2122$t_points_made/t_total_possessions*100,
         t_defensive_rating = team2122$o_points_made/t_total_possessions*100,
         
         t_expected_fg_percentage = (team2122$t_three_points_made + team2122$t_two_points_made +
                                       (0.5*team2122$t_three_points_made))/100,
         
         t_turnovers_percentage = (team2122$t_turnovers/
                                     (team2122$t_three_points_attempted + team2122$t_two_points_attempted +
                                        0.44*team2122$t_free_throws_attempted + team2122$t_turnovers)),
         o_turnovers_percentage = (team2122$o_turnovers/
                                     (team2122$o_three_points_attempted + team2122$o_two_points_attempted +
                                        0.44*team2122$o_free_throws_attempted + team2122$o_turnovers)),
         
         t_off_rebounds_percentage = team2122$t_offensive_rebounds/(team2122$t_offensive_rebounds + team2122$o_defensive_rebounds),
         
         t_def_rebounds_percentage = team2122$t_defensive_rebounds/(team2122$t_defensive_rebounds + team2122$o_offensive_rebounds),
         
         o_def_rebounds_percentage = team2122$o_defensive_rebounds/(team2122$o_defensive_rebounds + team2122$t_offensive_rebounds),
         
         o_off_rebounds_percentage = team2122$o_offensive_rebounds/(team2122$o_offensive_rebounds + team2122$t_defensive_rebounds)
  )


team2122[team2122 == "Inf"] <- NaN
team2122[team2122 == "-Inf"] <- NaN

players2122 <- left_join(
  nbb2122 %>% filter(name != "Equipe"),
  team2122 %>% select(id, period, starts_with("t_"), starts_with("o_")),
  by = c("id", "period", "t_minutes"))

#calculate individual scoring possessions
q5 <- 1.14*((players2122$t_assists - players2122$p_assists)/
              (players2122$t_two_points_made + players2122$t_three_points_made))

q12 <- ((players2122$t_assists/players2122$t_minutes)*players2122$p_minutes*5 - players2122$p_assists)/
  (((players2122$t_three_points_made + players2122$t_two_points_made)/players2122$t_minutes)*players2122$p_minutes*5 - players2122$p_three_points_made - players2122$p_two_points_made)

qast <- players2122$p_minutes/(players2122$t_minutes/5)*q5 + (1 - (players2122$p_minutes/(players2122$t_minutes/5)))*q12

fg_part <- (players2122$p_three_points_made + players2122$p_two_points_made)*(1-0.5*((
  players2122$p_points_made - players2122$p_free_throws_made)/
    (2 * (players2122$p_three_points_attempted + players2122$p_two_points_attempted)))*qast)

ast_part <- 0.5 * (((players2122$t_points_made - players2122$t_free_throws_made) -
                      (players2122$p_points_made - players2122$p_free_throws_made))/
                     (2 * (players2122$t_three_points_attempted + players2122$t_two_points_attempted - 
                             players2122$p_three_points_attempted - players2122$p_two_points_attempted)))*players2122$p_assists

ft_part <- (1 - (1 - players2122$p_free_throws_percentage)^2)*0.4*players2122$p_free_throws_attempted
ft_part <- gsub("NaN", 0, ft_part) %>% as.numeric()

tmor_weight <- ((1 - players2122$t_off_rebounds_percentage) * players2122$t_plays_percentage)/
  ((1 - players2122$t_off_rebounds_percentage) * players2122$t_plays_percentage + players2122$t_off_rebounds_percentage * 
     (1 - players2122$t_plays_percentage))

or_part <- players2122$p_offensive_rebounds * tmor_weight * players2122$t_plays_percentage

#calculate individual total possessions
missed_fg_part <- (players2122$p_three_points_attempted + players2122$p_two_points_attempted -
                     players2122$p_three_points_made - players2122$p_two_points_made)*(
                       1 - 1.07 * players2122$t_off_rebounds_percentage)

missed_ft_part <- (1 - players2122$p_free_throws_percentage)^2 * 0.4 *(players2122$p_free_throws_attempted)                     
missed_ft_part <- gsub("NaN", 0, missed_ft_part) %>% as.numeric()

#calculate individual points produced
fg_part_pp <- 2*(players2122$p_three_points_made + players2122$p_two_points_made + 0.5*players2122$p_three_points_made)*(
  1-0.5*((players2122$p_points_made - players2122$p_free_throws_made)/(2 * (players2122$p_three_points_attempted + players2122$p_two_points_attempted)))*qast)

ast_part_pp <- 2 * ((players2122$t_three_points_made + players2122$t_two_points_made - 
                       players2122$p_three_points_made - players2122$p_two_points_made +
                       0.5 * (players2122$t_three_points_made - players2122$p_three_points_made))/
                      (players2122$t_three_points_made + players2122$t_two_points_made - 
                         players2122$p_three_points_made - players2122$p_two_points_made))*0.5*(((
                           players2122$t_points_made - players2122$t_free_throws_made) - (
                             players2122$p_points_made - players2122$p_free_throws_made))/(
                               2 * (players2122$t_three_points_attempted + players2122$t_two_points_attempted - 
                                      players2122$p_three_points_attempted - players2122$p_two_points_attempted))) * players2122$p_assists

or_part_pp <- or_part * (players2122$t_points_made/(players2122$t_three_points_made + players2122$t_two_points_made +(
  1 - (1 - players2122$t_free_throws_percentage)^2)*0.4*players2122$t_free_throws_attempted))


players2122 <- players2122 %>% ungroup() %>% 
  mutate(p_scoring_possessions = ((fg_part + ast_part + ft_part) * 
                                    (1 - players2122$t_offensive_rebounds/players2122$t_scoring_possessions * tmor_weight * players2122$t_plays_percentage) + or_part),
         
         p_total_possessions = (p_scoring_possessions + missed_fg_part + missed_ft_part + players2122$p_turnovers),
         
         p_floor_percentage = (p_scoring_possessions/p_total_possessions),
         
         p_points_produced = ((fg_part_pp + ast_part_pp + players2122$p_free_throws_made) * (
           1 - players2122$t_offensive_rebounds/players2122$t_scoring_possessions * tmor_weight * players2122$t_plays_percentage) + or_part_pp),
         
         p_offensive_rating = (p_points_produced/p_total_possessions*100),
         
         p_possessions_percentage = (p_total_possessions/players2122$t_total_possessions))

#ADVANCED DEFENSIVE STATS

fmwt <- (players2122$o_field_goal_percentage * (1 - players2122$o_off_rebounds_percentage))/(
  players2122$o_field_goal_percentage * (1 - players2122$o_off_rebounds_percentage) + (
    1 - players2122$o_field_goal_percentage) * players2122$o_off_rebounds_percentage)

stops1 <- players2122$p_steals + players2122$p_blocks * fmwt * (1 - 1.07 *players2122$o_off_rebounds_percentage) +
  players2122$p_defensive_rebounds * (1 - fmwt)

stops2 <- (((players2122$o_three_points_attempted + players2122$o_two_points_attempted -
               players2122$o_three_points_made - players2122$o_two_points_made - players2122$t_blocks)/
              players2122$t_minutes) * fmwt * (1 - 1.07 * players2122$o_off_rebounds_percentage) + 
             ((players2122$o_turnovers - players2122$t_steals)/players2122$t_minutes)) * 
  players2122$p_minutes + (players2122$p_fouls_committed/players2122$t_fouls_committed) * 0.4 *
  players2122$o_free_throws_attempted * ((1 - players2122$o_free_throws_percentage)^2)

stops <- stops1 + stops2

p_def_pts_per_scposs <-  players2122$o_points_made/(players2122$o_three_points_made + players2122$o_two_points_made + (
  1 - (1 - ((players2122$o_three_points_made + players2122$o_two_points_made)/(
    players2122$o_three_points_attempted + players2122$o_two_points_attempted)))^2*
    players2122$o_free_throws_attempted*0.4))

players2122 <- players2122 %>% 
  mutate(p_stops_percentage = (stops * players2122$t_minutes)/(players2122$t_total_possessions * players2122$p_minutes),
         p_defensive_rating = players2122$t_defensive_rating + 0.2 * (100 * p_def_pts_per_scposs * (1 - p_stops_percentage) -
                                                                        players2122$t_defensive_rating))

players2122[players2122 == "Inf"] <- NaN
players2122[players2122 == "-Inf"] <- NaN
players2122 <- players2122 %>% 
...
sqlQuery(con, "CREATE TABLE `schedule2122` (
...
sqlQuery(con, "CREATE TABLE `team2122` (
         ...
         FOREIGN KEY (id) REFERENCES schedule2122(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;")

sqlQuery(con, "CREATE TABLE `players2122` (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlQuery(con, "CREATE TABLE period2122 (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlQuery(con, "CREATE TABLE `coach2122` (
...
sqlSave(con, schedule2122, "schedule2122", rownames = F, fast = T, append = T)
sqlSave(con, team2122, "team2122", rownames = F, fast = T, append = T)
sqlSave(con, players2122, "players2122", rownames = F, fast = T, append = T)
sqlSave(con, period2122, "period2122", rownames = F, fast = T, append = T)
sqlSave(con, coach2122, "coach2122", rownames = F, fast = T, append = T)
