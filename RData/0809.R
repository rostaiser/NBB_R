...
### download schedule
url <- read_html("https://lnb.com.br/nbb/tabela-de-jogos/?season%5B%5D=1")
...
schedule0809 <- tibble(id, home_away = rep(c("home","away"), length(date)/2), date, time, match, team, opponent, season_phase, season)

period0809 <- tibble(id, period = rep("game", length(id)),
                     score = c(rbind(home_score, away_score)) - c(rbind(away_score, home_score)))
period0809 <- period0809 %>% mutate(game_result = ifelse(score > 0, "won", "lost"))

links_0809 <- url %>% html_nodes(".match_score_relatorio") %>% html_attr("href")
links_0809 <- links_0809[c(T,F)]
links_0809 <- tibble(link = links_0809, id = c(1:length(links_0809)))

static_final <- tibble()

for (i in 1:nrow(links_0809)) {
  link <- links_0809$link[i]
  ...
  idh <- paste0(links_0809$id[i], "H")
  ida <- paste0(links_0809$id[i], "A")
...
nbb0809 <-  static_final %>% group_by(id, period) %>% 
...
home0809$id <- home0809$id %>%  str_replace_all("H","A")

away0809 <- team0809 %>% 
  subset(grepl("A", team0809$id)) %>% 
  rename(o_points_made = t_points_made,
         o_points_attempted = t_points_attempted,
         o_points_percentage = t_points_percentage,
         o_assists = t_assists,
...
away0809$id <- away0809$id %>%  str_replace_all("A","H")

opponent0809 <- rbind(home0809, away0809) %>% 
  arrange(id)

team0809 <- left_join(team0809, opponent0809) %>% 
  select(-name) %>% 
  arrange(id)

team0809 <- team0809 %>% ungroup() %>% 
  mutate(t_total_possessions = (((team0809$t_three_points_attempted + team0809$t_two_points_attempted +
                                    team0809$t_turnovers + (0.4*team0809$t_free_throws_attempted) -
                                    (team0809$t_offensive_rebounds/(team0809$t_offensive_rebounds + team0809$o_defensive_rebounds))*
                                    ((team0809$t_three_points_attempted + team0809$t_two_points_attempted) -
                                       (team0809$t_three_points_made + team0809$t_two_points_made))*1.07) + (
                                         team0809$o_three_points_attempted + team0809$o_two_points_attempted +
                                           team0809$o_turnovers + (0.4*team0809$o_free_throws_attempted) -
                                           (team0809$o_offensive_rebounds/(team0809$o_offensive_rebounds + team0809$t_defensive_rebounds))*
                                           ((team0809$o_three_points_attempted + team0809$o_two_points_attempted) -
                                              (team0809$o_three_points_made + team0809$o_two_points_made))*1.07 ))/2),
         
         t_scoring_possessions = (team0809$t_three_points_made + team0809$t_two_points_made +
                                    (1 - (1-team0809$t_free_throws_percentage)^2)*team0809$t_free_throws_attempted*0.4),
         
         t_floor_percentage = t_scoring_possessions/t_total_possessions,
         
         t_plays = (team0809$t_three_points_attempted + team0809$t_two_points_attempted +
                      team0809$t_free_throws_attempted*0.4 + team0809$t_turnovers),
         
         t_plays_percentage = t_scoring_possessions/t_plays,
         
         t_field_percentage = (team0809$t_three_points_made + team0809$t_two_points_made)/
           ((team0809$t_three_points_attempted + team0809$t_two_points_attempted - team0809$t_offensive_rebounds)/
              (team0809$t_offensive_rebounds + team0809$o_defensive_rebounds)*
              (team0809$t_three_points_attempted + team0809$t_two_points_attempted -
                 team0809$t_three_points_made - team0809$t_two_points_made)*1.07 +
              team0809$t_turnovers),
         
         t_offensive_rating = team0809$t_points_made/t_total_possessions*100,
         t_defensive_rating = team0809$o_points_made/t_total_possessions*100,
         
         t_expected_fg_percentage = (team0809$t_three_points_made + team0809$t_two_points_made +
                                       (0.5*team0809$t_three_points_made))/100,
         
         t_turnovers_percentage = (team0809$t_turnovers/
                                     (team0809$t_three_points_attempted + team0809$t_two_points_attempted +
                                        0.44*team0809$t_free_throws_attempted + team0809$t_turnovers)),
         o_turnovers_percentage = (team0809$o_turnovers/
                                     (team0809$o_three_points_attempted + team0809$o_two_points_attempted +
                                        0.44*team0809$o_free_throws_attempted + team0809$o_turnovers)),
         
         t_off_rebounds_percentage = team0809$t_offensive_rebounds/(team0809$t_offensive_rebounds + team0809$o_defensive_rebounds),
         
         t_def_rebounds_percentage = team0809$t_defensive_rebounds/(team0809$t_defensive_rebounds + team0809$o_offensive_rebounds),
         
         o_def_rebounds_percentage = team0809$o_defensive_rebounds/(team0809$o_defensive_rebounds + team0809$t_offensive_rebounds),
         
         o_off_rebounds_percentage = team0809$o_offensive_rebounds/(team0809$o_offensive_rebounds + team0809$t_defensive_rebounds)
  )


team0809[team0809 == "Inf"] <- NaN
team0809[team0809 == "-Inf"] <- NaN

players0809 <- left_join(
  nbb0809 %>% filter(name != "Equipe"),
  team0809 %>% select(id, period, starts_with("t_"), starts_with("o_")),
  by = c("id", "period", "t_minutes"))

#calculate individual scoring possessions
q5 <- 1.14*((players0809$t_assists - players0809$p_assists)/
              (players0809$t_two_points_made + players0809$t_three_points_made))

q12 <- ((players0809$t_assists/players0809$t_minutes)*players0809$p_minutes*5 - players0809$p_assists)/
  (((players0809$t_three_points_made + players0809$t_two_points_made)/players0809$t_minutes)*players0809$p_minutes*5 - players0809$p_three_points_made - players0809$p_two_points_made)

qast <- players0809$p_minutes/(players0809$t_minutes/5)*q5 + (1 - (players0809$p_minutes/(players0809$t_minutes/5)))*q12

fg_part <- (players0809$p_three_points_made + players0809$p_two_points_made)*(1-0.5*((
  players0809$p_points_made - players0809$p_free_throws_made)/
    (2 * (players0809$p_three_points_attempted + players0809$p_two_points_attempted)))*qast)

ast_part <- 0.5 * (((players0809$t_points_made - players0809$t_free_throws_made) -
                      (players0809$p_points_made - players0809$p_free_throws_made))/
                     (2 * (players0809$t_three_points_attempted + players0809$t_two_points_attempted - 
                             players0809$p_three_points_attempted - players0809$p_two_points_attempted)))*players0809$p_assists

ft_part <- (1 - (1 - players0809$p_free_throws_percentage)^2)*0.4*players0809$p_free_throws_attempted
ft_part <- gsub("NaN", 0, ft_part) %>% as.numeric()

tmor_weight <- ((1 - players0809$t_off_rebounds_percentage) * players0809$t_plays_percentage)/
  ((1 - players0809$t_off_rebounds_percentage) * players0809$t_plays_percentage + players0809$t_off_rebounds_percentage * 
     (1 - players0809$t_plays_percentage))

or_part <- players0809$p_offensive_rebounds * tmor_weight * players0809$t_plays_percentage

#calculate individual total possessions
missed_fg_part <- (players0809$p_three_points_attempted + players0809$p_two_points_attempted -
                     players0809$p_three_points_made - players0809$p_two_points_made)*(
                       1 - 1.07 * players0809$t_off_rebounds_percentage)

missed_ft_part <- (1 - players0809$p_free_throws_percentage)^2 * 0.4 *(players0809$p_free_throws_attempted)                     
missed_ft_part <- gsub("NaN", 0, missed_ft_part) %>% as.numeric()

#calculate individual points produced
fg_part_pp <- 2*(players0809$p_three_points_made + players0809$p_two_points_made + 0.5*players0809$p_three_points_made)*(
  1-0.5*((players0809$p_points_made - players0809$p_free_throws_made)/(2 * (players0809$p_three_points_attempted + players0809$p_two_points_attempted)))*qast)

ast_part_pp <- 2 * ((players0809$t_three_points_made + players0809$t_two_points_made - 
                       players0809$p_three_points_made - players0809$p_two_points_made +
                       0.5 * (players0809$t_three_points_made - players0809$p_three_points_made))/
                      (players0809$t_three_points_made + players0809$t_two_points_made - 
                         players0809$p_three_points_made - players0809$p_two_points_made))*0.5*(((
                           players0809$t_points_made - players0809$t_free_throws_made) - (
                             players0809$p_points_made - players0809$p_free_throws_made))/(
                               2 * (players0809$t_three_points_attempted + players0809$t_two_points_attempted - 
                                      players0809$p_three_points_attempted - players0809$p_two_points_attempted))) * players0809$p_assists

or_part_pp <- or_part * (players0809$t_points_made/(players0809$t_three_points_made + players0809$t_two_points_made +(
  1 - (1 - players0809$t_free_throws_percentage)^2)*0.4*players0809$t_free_throws_attempted))


players0809 <- players0809 %>% ungroup() %>% 
  mutate(p_scoring_possessions = ((fg_part + ast_part + ft_part) * 
                                    (1 - players0809$t_offensive_rebounds/players0809$t_scoring_possessions * tmor_weight * players0809$t_plays_percentage) + or_part),
         
         p_total_possessions = (p_scoring_possessions + missed_fg_part + missed_ft_part + players0809$p_turnovers),
         
         p_floor_percentage = (p_scoring_possessions/p_total_possessions),
         
         p_points_produced = ((fg_part_pp + ast_part_pp + players0809$p_free_throws_made) * (
           1 - players0809$t_offensive_rebounds/players0809$t_scoring_possessions * tmor_weight * players0809$t_plays_percentage) + or_part_pp),
         
         p_offensive_rating = (p_points_produced/p_total_possessions*100),
         
         p_possessions_percentage = (p_total_possessions/players0809$t_total_possessions))

#ADVANCED DEFENSIVE STATS

fmwt <- (players0809$o_field_goal_percentage * (1 - players0809$o_off_rebounds_percentage))/(
  players0809$o_field_goal_percentage * (1 - players0809$o_off_rebounds_percentage) + (
    1 - players0809$o_field_goal_percentage) * players0809$o_off_rebounds_percentage)

stops1 <- players0809$p_steals + players0809$p_blocks * fmwt * (1 - 1.07 *players0809$o_off_rebounds_percentage) +
  players0809$p_defensive_rebounds * (1 - fmwt)

stops2 <- (((players0809$o_three_points_attempted + players0809$o_two_points_attempted -
               players0809$o_three_points_made - players0809$o_two_points_made - players0809$t_blocks)/
              players0809$t_minutes) * fmwt * (1 - 1.07 * players0809$o_off_rebounds_percentage) + 
             ((players0809$o_turnovers - players0809$t_steals)/players0809$t_minutes)) * 
  players0809$p_minutes + (players0809$p_fouls_committed/players0809$t_fouls_committed) * 0.4 *
  players0809$o_free_throws_attempted * ((1 - players0809$o_free_throws_percentage)^2)

stops <- stops1 + stops2

p_def_pts_per_scposs <-  players0809$o_points_made/(players0809$o_three_points_made + players0809$o_two_points_made + (
  1 - (1 - ((players0809$o_three_points_made + players0809$o_two_points_made)/(
    players0809$o_three_points_attempted + players0809$o_two_points_attempted)))^2*
    players0809$o_free_throws_attempted*0.4))

players0809 <- players0809 %>% 
  mutate(p_stops_percentage = (stops * players0809$t_minutes)/(players0809$t_total_possessions * players0809$p_minutes),
         p_defensive_rating = players0809$t_defensive_rating + 0.2 * (100 * p_def_pts_per_scposs * (1 - p_stops_percentage) -
                                                                        players0809$t_defensive_rating))

players0809[players0809 == "Inf"] <- NaN
players0809[players0809 == "-Inf"] <- NaN
players0809 <- players0809 %>% 
...
sqlQuery(con, "CREATE TABLE `schedule0809` (
...
sqlQuery(con, "CREATE TABLE `team0809` (
         ...
         FOREIGN KEY (id) REFERENCES schedule0809(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;")

sqlQuery(con, "CREATE TABLE `players0809` (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
         `jersey` varchar(5) NOT NULL,
...
sqlQuery(con, "CREATE TABLE period0809 (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlSave(con, schedule0809, "schedule0809", rownames = F, fast = T, append = T)
sqlSave(con, team0809, "team0809", rownames = F, fast = T, append = T)
sqlSave(con, players0809, "players0809", rownames = F, fast = T, append = T)
sqlSave(con, period0809, "period0809", rownames = F, fast = T, append = T)
