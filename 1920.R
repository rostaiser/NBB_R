...
### download schedule
url <- read_html("https://lnb.com.br/nbb/tabela-de-jogos/?season%5B%5D=54")
...
schedule1920 <- tibble(id, home_away = rep(c("home","away"), length(date)/2), date, time, match, team, opponent, season_phase, season, venue)
...
links_1920 <- url %>% html_nodes(".match_score_relatorio") %>% html_attr("href")
links_1920 <- links_1920[c(T,F)]
links_1920 <- tibble(link = links_1920, id = c(1:length(links_1920)))
...
for (i in 1:nrow(links_1920)) {
  link <- links_1920$link[i]
  if (grepl("partida", link) == F) {
    ...
    idh <- paste0(links_1920$id[i], "H")
    ida <- paste0(links_1920$id[i], "A")
...
period1920 <- rbind(period_static, period_dynamic) %>% 
  arrange(id)
period1920 <- period1920 %>% 
  ...
  fill(game_result)
coach1920 <- rbind(coach_static, coach_dynamic) %>% 
  arrange(id)
...
nbb1920 <- rbind(dynamic_final, static_final)
...
home1920$id <- home1920$id %>%  str_replace_all("H","A")

away1920 <- team1920 %>% 
  subset(grepl("A", team1920$id)) %>% 
  rename(o_points_made = t_points_made,
         o_points_attempted = t_points_attempted,
         o_points_percentage = t_points_percentage,
         o_assists = t_assists,
...
away1920$id <- away1920$id %>%  str_replace_all("A","H")

opponent1920 <- rbind(home1920, away1920) %>% 
  arrange(id)

team1920 <- left_join(team1920, opponent1920) %>% 
  select(-name) %>% 
  arrange(id)

team1920 <- team1920 %>% ungroup() %>% 
  mutate(t_total_possessions = (((team1920$t_three_points_attempted + team1920$t_two_points_attempted +
                                    team1920$t_turnovers + (0.4*team1920$t_free_throws_attempted) -
                                    (team1920$t_offensive_rebounds/(team1920$t_offensive_rebounds + team1920$o_defensive_rebounds))*
                                    ((team1920$t_three_points_attempted + team1920$t_two_points_attempted) -
                                       (team1920$t_three_points_made + team1920$t_two_points_made))*1.07) + (
                                         team1920$o_three_points_attempted + team1920$o_two_points_attempted +
                                           team1920$o_turnovers + (0.4*team1920$o_free_throws_attempted) -
                                           (team1920$o_offensive_rebounds/(team1920$o_offensive_rebounds + team1920$t_defensive_rebounds))*
                                           ((team1920$o_three_points_attempted + team1920$o_two_points_attempted) -
                                              (team1920$o_three_points_made + team1920$o_two_points_made))*1.07 ))/2),
         
         t_scoring_possessions = (team1920$t_three_points_made + team1920$t_two_points_made +
                                    (1 - (1-team1920$t_free_throws_percentage)^2)*team1920$t_free_throws_attempted*0.4),
         
         t_floor_percentage = t_scoring_possessions/t_total_possessions,
         
         t_plays = (team1920$t_three_points_attempted + team1920$t_two_points_attempted +
                      team1920$t_free_throws_attempted*0.4 + team1920$t_turnovers),
         
         t_plays_percentage = t_scoring_possessions/t_plays,
         
         t_field_percentage = (team1920$t_three_points_made + team1920$t_two_points_made)/
           ((team1920$t_three_points_attempted + team1920$t_two_points_attempted - team1920$t_offensive_rebounds)/
              (team1920$t_offensive_rebounds + team1920$o_defensive_rebounds)*
              (team1920$t_three_points_attempted + team1920$t_two_points_attempted -
                 team1920$t_three_points_made - team1920$t_two_points_made)*1.07 +
              team1920$t_turnovers),
         
         t_offensive_rating = team1920$t_points_made/t_total_possessions*100,
         t_defensive_rating = team1920$o_points_made/t_total_possessions*100,
         
         t_expected_fg_percentage = (team1920$t_three_points_made + team1920$t_two_points_made +
                                       (0.5*team1920$t_three_points_made))/100,
         
         t_turnovers_percentage = (team1920$t_turnovers/
                                     (team1920$t_three_points_attempted + team1920$t_two_points_attempted +
                                        0.44*team1920$t_free_throws_attempted + team1920$t_turnovers)),
         o_turnovers_percentage = (team1920$o_turnovers/
                                     (team1920$o_three_points_attempted + team1920$o_two_points_attempted +
                                        0.44*team1920$o_free_throws_attempted + team1920$o_turnovers)),
         
         t_off_rebounds_percentage = team1920$t_offensive_rebounds/(team1920$t_offensive_rebounds + team1920$o_defensive_rebounds),
         
         t_def_rebounds_percentage = team1920$t_defensive_rebounds/(team1920$t_defensive_rebounds + team1920$o_offensive_rebounds),
         
         o_def_rebounds_percentage = team1920$o_defensive_rebounds/(team1920$o_defensive_rebounds + team1920$t_offensive_rebounds),
         
         o_off_rebounds_percentage = team1920$o_offensive_rebounds/(team1920$o_offensive_rebounds + team1920$t_defensive_rebounds)
  )


team1920[team1920 == "Inf"] <- NaN
team1920[team1920 == "-Inf"] <- NaN

players1920 <- left_join(
  nbb1920 %>% filter(name != "Equipe"),
  team1920 %>% select(id, period, starts_with("t_"), starts_with("o_")),
  by = c("id", "period", "t_minutes"))

#calculate individual scoring possessions
q5 <- 1.14*((players1920$t_assists - players1920$p_assists)/
              (players1920$t_two_points_made + players1920$t_three_points_made))

q12 <- ((players1920$t_assists/players1920$t_minutes)*players1920$p_minutes*5 - players1920$p_assists)/
  (((players1920$t_three_points_made + players1920$t_two_points_made)/players1920$t_minutes)*players1920$p_minutes*5 - players1920$p_three_points_made - players1920$p_two_points_made)

qast <- players1920$p_minutes/(players1920$t_minutes/5)*q5 + (1 - (players1920$p_minutes/(players1920$t_minutes/5)))*q12

fg_part <- (players1920$p_three_points_made + players1920$p_two_points_made)*(1-0.5*((
  players1920$p_points_made - players1920$p_free_throws_made)/
    (2 * (players1920$p_three_points_attempted + players1920$p_two_points_attempted)))*qast)

ast_part <- 0.5 * (((players1920$t_points_made - players1920$t_free_throws_made) -
                      (players1920$p_points_made - players1920$p_free_throws_made))/
                     (2 * (players1920$t_three_points_attempted + players1920$t_two_points_attempted - 
                             players1920$p_three_points_attempted - players1920$p_two_points_attempted)))*players1920$p_assists

ft_part <- (1 - (1 - players1920$p_free_throws_percentage)^2)*0.4*players1920$p_free_throws_attempted
ft_part <- gsub("NaN", 0, ft_part) %>% as.numeric()

tmor_weight <- ((1 - players1920$t_off_rebounds_percentage) * players1920$t_plays_percentage)/
  ((1 - players1920$t_off_rebounds_percentage) * players1920$t_plays_percentage + players1920$t_off_rebounds_percentage * 
     (1 - players1920$t_plays_percentage))

or_part <- players1920$p_offensive_rebounds * tmor_weight * players1920$t_plays_percentage

#calculate individual total possessions
missed_fg_part <- (players1920$p_three_points_attempted + players1920$p_two_points_attempted -
                     players1920$p_three_points_made - players1920$p_two_points_made)*(
                       1 - 1.07 * players1920$t_off_rebounds_percentage)

missed_ft_part <- (1 - players1920$p_free_throws_percentage)^2 * 0.4 *(players1920$p_free_throws_attempted)                     
missed_ft_part <- gsub("NaN", 0, missed_ft_part) %>% as.numeric()

#calculate individual points produced
fg_part_pp <- 2*(players1920$p_three_points_made + players1920$p_two_points_made + 0.5*players1920$p_three_points_made)*(
  1-0.5*((players1920$p_points_made - players1920$p_free_throws_made)/(2 * (players1920$p_three_points_attempted + players1920$p_two_points_attempted)))*qast)

ast_part_pp <- 2 * ((players1920$t_three_points_made + players1920$t_two_points_made - 
                       players1920$p_three_points_made - players1920$p_two_points_made +
                       0.5 * (players1920$t_three_points_made - players1920$p_three_points_made))/
                      (players1920$t_three_points_made + players1920$t_two_points_made - 
                         players1920$p_three_points_made - players1920$p_two_points_made))*0.5*(((
                           players1920$t_points_made - players1920$t_free_throws_made) - (
                             players1920$p_points_made - players1920$p_free_throws_made))/(
                               2 * (players1920$t_three_points_attempted + players1920$t_two_points_attempted - 
                                      players1920$p_three_points_attempted - players1920$p_two_points_attempted))) * players1920$p_assists

or_part_pp <- or_part * (players1920$t_points_made/(players1920$t_three_points_made + players1920$t_two_points_made +(
  1 - (1 - players1920$t_free_throws_percentage)^2)*0.4*players1920$t_free_throws_attempted))


players1920 <- players1920 %>% ungroup() %>% 
  mutate(p_scoring_possessions = ((fg_part + ast_part + ft_part) * 
                                    (1 - players1920$t_offensive_rebounds/players1920$t_scoring_possessions * tmor_weight * players1920$t_plays_percentage) + or_part),
         
         p_total_possessions = (p_scoring_possessions + missed_fg_part + missed_ft_part + players1920$p_turnovers),
         
         p_floor_percentage = (p_scoring_possessions/p_total_possessions),
         
         p_points_produced = ((fg_part_pp + ast_part_pp + players1920$p_free_throws_made) * (
           1 - players1920$t_offensive_rebounds/players1920$t_scoring_possessions * tmor_weight * players1920$t_plays_percentage) + or_part_pp),
         
         p_offensive_rating = (p_points_produced/p_total_possessions*100),
         
         p_possessions_percentage = (p_total_possessions/players1920$t_total_possessions))

#ADVANCED DEFENSIVE STATS

fmwt <- (players1920$o_field_goal_percentage * (1 - players1920$o_off_rebounds_percentage))/(
  players1920$o_field_goal_percentage * (1 - players1920$o_off_rebounds_percentage) + (
    1 - players1920$o_field_goal_percentage) * players1920$o_off_rebounds_percentage)

stops1 <- players1920$p_steals + players1920$p_blocks * fmwt * (1 - 1.07 *players1920$o_off_rebounds_percentage) +
  players1920$p_defensive_rebounds * (1 - fmwt)

stops2 <- (((players1920$o_three_points_attempted + players1920$o_two_points_attempted -
               players1920$o_three_points_made - players1920$o_two_points_made - players1920$t_blocks)/
              players1920$t_minutes) * fmwt * (1 - 1.07 * players1920$o_off_rebounds_percentage) + 
             ((players1920$o_turnovers - players1920$t_steals)/players1920$t_minutes)) * 
  players1920$p_minutes + (players1920$p_fouls_committed/players1920$t_fouls_committed) * 0.4 *
  players1920$o_free_throws_attempted * ((1 - players1920$o_free_throws_percentage)^2)

stops <- stops1 + stops2

p_def_pts_per_scposs <-  players1920$o_points_made/(players1920$o_three_points_made + players1920$o_two_points_made + (
  1 - (1 - ((players1920$o_three_points_made + players1920$o_two_points_made)/(
    players1920$o_three_points_attempted + players1920$o_two_points_attempted)))^2*
    players1920$o_free_throws_attempted*0.4))

players1920 <- players1920 %>% 
  mutate(p_stops_percentage = (stops * players1920$t_minutes)/(players1920$t_total_possessions * players1920$p_minutes),
         p_defensive_rating = players1920$t_defensive_rating + 0.2 * (100 * p_def_pts_per_scposs * (1 - p_stops_percentage) -
                                                                        players1920$t_defensive_rating))

players1920[players1920 == "Inf"] <- NaN
players1920[players1920 == "-Inf"] <- NaN
players1920 <- players1920 %>% 
...
sqlQuery(con, "CREATE TABLE `schedule1920` (
...
sqlQuery(con, "CREATE TABLE `team1920` (
         ...
         FOREIGN KEY (id) REFERENCES schedule1920(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;")

sqlQuery(con, "CREATE TABLE `players1920` (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlQuery(con, "CREATE TABLE period1920 (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlQuery(con, "CREATE TABLE `coach1920` (
...
sqlSave(con, schedule1920, "schedule1920", rownames = F, fast = T, append = T)
sqlSave(con, team1920, "team1920", rownames = F, fast = T, append = T)
sqlSave(con, players1920, "players1920", rownames = F, fast = T, append = T)
sqlSave(con, period1920, "period1920", rownames = F, fast = T, append = T)
sqlSave(con, coach1920, "coach1920", rownames = F, fast = T, append = T)
