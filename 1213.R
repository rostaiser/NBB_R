...
### download schedule
url <- read_html("https://lnb.com.br/nbb/tabela-de-jogos/?season%5B%5D=8")
...
schedule1213 <- tibble(id, home_away = rep(c("home","away"), length(date)/2), date, time, match, team, opponent, season_phase, season, venue)

period1213 <- tibble(id, period = rep("game", length(id)),
                     score = c(rbind(home_score, away_score)) - c(rbind(away_score, home_score)))
period1213 <- period1213 %>% mutate(game_result = ifelse(score > 0, "won", "lost"))

links_1213 <- url %>% html_nodes(".match_score_relatorio") %>% html_attr("href")
links_1213 <- links_1213[c(T,F)]
links_1213 <- tibble(link = links_1213, id = c(1:length(links_1213)))
links_1213 <- links_1213[-c(1,7,16,17,25,29,30,35,39,43,44,46,52,56,60,78,116,130,135,147,176,184,198
                            ),]

static_final <- tibble()

for (i in 1:nrow(links_1213)) {
  if (links_1213$id[i] <= 253) {
  link <- links_1213$link[i]
  ...
  idh <- paste0(links_1213$id[i], "H")
  ida <- paste0(links_1213$id[i], "A")
  ...
  stats <- tibble(id = c(rep(idh, nhome*l),rep(ida, naway*l)),
    ...
    web <- read_html(link)
    stats <- web %>% html_nodes(".tablesorter") %>% html_table()
    stats <- stats[c(1,(length(stats)/2)+1)]
...
nbb1213 <-  static_final %>% group_by(id, period) %>% 
...
home1213$id <- home1213$id %>%  str_replace_all("H","A")

away1213 <- team1213 %>% 
  subset(grepl("A", team1213$id)) %>% 
  rename(o_points_made = t_points_made,
         o_points_attempted = t_points_attempted,
         o_points_percentage = t_points_percentage,
         o_assists = t_assists,
...
away1213$id <- away1213$id %>%  str_replace_all("A","H")

opponent1213 <- rbind(home1213, away1213) %>% 
  arrange(id)

team1213 <- left_join(team1213, opponent1213) %>% 
  select(-name) %>% 
  arrange(id)

team1213 <- team1213 %>% ungroup() %>% 
  mutate(t_total_possessions = (((team1213$t_three_points_attempted + team1213$t_two_points_attempted +
                                    team1213$t_turnovers + (0.4*team1213$t_free_throws_attempted) -
                                    (team1213$t_offensive_rebounds/(team1213$t_offensive_rebounds + team1213$o_defensive_rebounds))*
                                    ((team1213$t_three_points_attempted + team1213$t_two_points_attempted) -
                                       (team1213$t_three_points_made + team1213$t_two_points_made))*1.07) + (
                                         team1213$o_three_points_attempted + team1213$o_two_points_attempted +
                                           team1213$o_turnovers + (0.4*team1213$o_free_throws_attempted) -
                                           (team1213$o_offensive_rebounds/(team1213$o_offensive_rebounds + team1213$t_defensive_rebounds))*
                                           ((team1213$o_three_points_attempted + team1213$o_two_points_attempted) -
                                              (team1213$o_three_points_made + team1213$o_two_points_made))*1.07 ))/2),
         
         t_scoring_possessions = (team1213$t_three_points_made + team1213$t_two_points_made +
                                    (1 - (1-team1213$t_free_throws_percentage)^2)*team1213$t_free_throws_attempted*0.4),
         
         t_floor_percentage = t_scoring_possessions/t_total_possessions,
         
         t_plays = (team1213$t_three_points_attempted + team1213$t_two_points_attempted +
                      team1213$t_free_throws_attempted*0.4 + team1213$t_turnovers),
         
         t_plays_percentage = t_scoring_possessions/t_plays,
         
         t_field_percentage = (team1213$t_three_points_made + team1213$t_two_points_made)/
           ((team1213$t_three_points_attempted + team1213$t_two_points_attempted - team1213$t_offensive_rebounds)/
              (team1213$t_offensive_rebounds + team1213$o_defensive_rebounds)*
              (team1213$t_three_points_attempted + team1213$t_two_points_attempted -
                 team1213$t_three_points_made - team1213$t_two_points_made)*1.07 +
              team1213$t_turnovers),
         
         t_offensive_rating = team1213$t_points_made/t_total_possessions*100,
         t_defensive_rating = team1213$o_points_made/t_total_possessions*100,
         
         t_expected_fg_percentage = (team1213$t_three_points_made + team1213$t_two_points_made +
                                       (0.5*team1213$t_three_points_made))/100,
         
         t_turnovers_percentage = (team1213$t_turnovers/
                                     (team1213$t_three_points_attempted + team1213$t_two_points_attempted +
                                        0.44*team1213$t_free_throws_attempted + team1213$t_turnovers)),
         o_turnovers_percentage = (team1213$o_turnovers/
                                     (team1213$o_three_points_attempted + team1213$o_two_points_attempted +
                                        0.44*team1213$o_free_throws_attempted + team1213$o_turnovers)),
         
         t_off_rebounds_percentage = team1213$t_offensive_rebounds/(team1213$t_offensive_rebounds + team1213$o_defensive_rebounds),
         
         t_def_rebounds_percentage = team1213$t_defensive_rebounds/(team1213$t_defensive_rebounds + team1213$o_offensive_rebounds),
         
         o_def_rebounds_percentage = team1213$o_defensive_rebounds/(team1213$o_defensive_rebounds + team1213$t_offensive_rebounds),
         
         o_off_rebounds_percentage = team1213$o_offensive_rebounds/(team1213$o_offensive_rebounds + team1213$t_defensive_rebounds)
  )


team1213[team1213 == "Inf"] <- NaN
team1213[team1213 == "-Inf"] <- NaN

players1213 <- left_join(
  nbb1213 %>% filter(name != "Equipe"),
  team1213 %>% select(id, period, starts_with("t_"), starts_with("o_")),
  by = c("id", "period", "t_minutes"))

#calculate individual scoring possessions
q5 <- 1.14*((players1213$t_assists - players1213$p_assists)/
              (players1213$t_two_points_made + players1213$t_three_points_made))

q12 <- ((players1213$t_assists/players1213$t_minutes)*players1213$p_minutes*5 - players1213$p_assists)/
  (((players1213$t_three_points_made + players1213$t_two_points_made)/players1213$t_minutes)*players1213$p_minutes*5 - players1213$p_three_points_made - players1213$p_two_points_made)

qast <- players1213$p_minutes/(players1213$t_minutes/5)*q5 + (1 - (players1213$p_minutes/(players1213$t_minutes/5)))*q12

fg_part <- (players1213$p_three_points_made + players1213$p_two_points_made)*(1-0.5*((
  players1213$p_points_made - players1213$p_free_throws_made)/
    (2 * (players1213$p_three_points_attempted + players1213$p_two_points_attempted)))*qast)

ast_part <- 0.5 * (((players1213$t_points_made - players1213$t_free_throws_made) -
                      (players1213$p_points_made - players1213$p_free_throws_made))/
                     (2 * (players1213$t_three_points_attempted + players1213$t_two_points_attempted - 
                             players1213$p_three_points_attempted - players1213$p_two_points_attempted)))*players1213$p_assists

ft_part <- (1 - (1 - players1213$p_free_throws_percentage)^2)*0.4*players1213$p_free_throws_attempted
ft_part <- gsub("NaN", 0, ft_part) %>% as.numeric()

tmor_weight <- ((1 - players1213$t_off_rebounds_percentage) * players1213$t_plays_percentage)/
  ((1 - players1213$t_off_rebounds_percentage) * players1213$t_plays_percentage + players1213$t_off_rebounds_percentage * 
     (1 - players1213$t_plays_percentage))

or_part <- players1213$p_offensive_rebounds * tmor_weight * players1213$t_plays_percentage

#calculate individual total possessions
missed_fg_part <- (players1213$p_three_points_attempted + players1213$p_two_points_attempted -
                     players1213$p_three_points_made - players1213$p_two_points_made)*(
                       1 - 1.07 * players1213$t_off_rebounds_percentage)

missed_ft_part <- (1 - players1213$p_free_throws_percentage)^2 * 0.4 *(players1213$p_free_throws_attempted)                     
missed_ft_part <- gsub("NaN", 0, missed_ft_part) %>% as.numeric()

#calculate individual points produced
fg_part_pp <- 2*(players1213$p_three_points_made + players1213$p_two_points_made + 0.5*players1213$p_three_points_made)*(
  1-0.5*((players1213$p_points_made - players1213$p_free_throws_made)/(2 * (players1213$p_three_points_attempted + players1213$p_two_points_attempted)))*qast)

ast_part_pp <- 2 * ((players1213$t_three_points_made + players1213$t_two_points_made - 
                       players1213$p_three_points_made - players1213$p_two_points_made +
                       0.5 * (players1213$t_three_points_made - players1213$p_three_points_made))/
                      (players1213$t_three_points_made + players1213$t_two_points_made - 
                         players1213$p_three_points_made - players1213$p_two_points_made))*0.5*(((
                           players1213$t_points_made - players1213$t_free_throws_made) - (
                             players1213$p_points_made - players1213$p_free_throws_made))/(
                               2 * (players1213$t_three_points_attempted + players1213$t_two_points_attempted - 
                                      players1213$p_three_points_attempted - players1213$p_two_points_attempted))) * players1213$p_assists

or_part_pp <- or_part * (players1213$t_points_made/(players1213$t_three_points_made + players1213$t_two_points_made +(
  1 - (1 - players1213$t_free_throws_percentage)^2)*0.4*players1213$t_free_throws_attempted))


players1213 <- players1213 %>% ungroup() %>% 
  mutate(p_scoring_possessions = ((fg_part + ast_part + ft_part) * 
                                    (1 - players1213$t_offensive_rebounds/players1213$t_scoring_possessions * tmor_weight * players1213$t_plays_percentage) + or_part),
         
         p_total_possessions = (p_scoring_possessions + missed_fg_part + missed_ft_part + players1213$p_turnovers),
         
         p_floor_percentage = (p_scoring_possessions/p_total_possessions),
         
         p_points_produced = ((fg_part_pp + ast_part_pp + players1213$p_free_throws_made) * (
           1 - players1213$t_offensive_rebounds/players1213$t_scoring_possessions * tmor_weight * players1213$t_plays_percentage) + or_part_pp),
         
         p_offensive_rating = (p_points_produced/p_total_possessions*100),
         
         p_possessions_percentage = (p_total_possessions/players1213$t_total_possessions))

#ADVANCED DEFENSIVE STATS

fmwt <- (players1213$o_field_goal_percentage * (1 - players1213$o_off_rebounds_percentage))/(
  players1213$o_field_goal_percentage * (1 - players1213$o_off_rebounds_percentage) + (
    1 - players1213$o_field_goal_percentage) * players1213$o_off_rebounds_percentage)

stops1 <- players1213$p_steals + players1213$p_blocks * fmwt * (1 - 1.07 *players1213$o_off_rebounds_percentage) +
  players1213$p_defensive_rebounds * (1 - fmwt)

stops2 <- (((players1213$o_three_points_attempted + players1213$o_two_points_attempted -
               players1213$o_three_points_made - players1213$o_two_points_made - players1213$t_blocks)/
              players1213$t_minutes) * fmwt * (1 - 1.07 * players1213$o_off_rebounds_percentage) + 
             ((players1213$o_turnovers - players1213$t_steals)/players1213$t_minutes)) * 
  players1213$p_minutes + (players1213$p_fouls_committed/players1213$t_fouls_committed) * 0.4 *
  players1213$o_free_throws_attempted * ((1 - players1213$o_free_throws_percentage)^2)

stops <- stops1 + stops2

p_def_pts_per_scposs <-  players1213$o_points_made/(players1213$o_three_points_made + players1213$o_two_points_made + (
  1 - (1 - ((players1213$o_three_points_made + players1213$o_two_points_made)/(
    players1213$o_three_points_attempted + players1213$o_two_points_attempted)))^2*
    players1213$o_free_throws_attempted*0.4))

players1213 <- players1213 %>% 
  mutate(p_stops_percentage = (stops * players1213$t_minutes)/(players1213$t_total_possessions * players1213$p_minutes),
         p_defensive_rating = players1213$t_defensive_rating + 0.2 * (100 * p_def_pts_per_scposs * (1 - p_stops_percentage) -
                                                                        players1213$t_defensive_rating))

players1213[players1213 == "Inf"] <- NaN
players1213[players1213 == "-Inf"] <- NaN
players1213 <- players1213 %>% 
...
sqlQuery(con, "CREATE TABLE `schedule1213` (
...
sqlQuery(con, "CREATE TABLE `team1213` (
         ...
         FOREIGN KEY (id) REFERENCES schedule1213(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;")

sqlQuery(con, "CREATE TABLE `players1213` (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlQuery(con, "CREATE TABLE period1213 (
         `id` varchar(4) NOT NULL,
         `period` varchar(11) NOT NULL,
...
sqlSave(con, schedule1213, "schedule1213", rownames = F, fast = T, append = T)
sqlSave(con, team1213, "team1213", rownames = F, fast = T, append = T)
sqlSave(con, players1213, "players1213", rownames = F, fast = T, append = T)
sqlSave(con, period1213, "period1213", rownames = F, fast = T, append = T)
